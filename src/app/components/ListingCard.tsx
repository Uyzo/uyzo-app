...
import { priceStr, INDEX_STYLE, KIND_EMOJI } from "@/lib/format";
import { t, type Lang } from "@/lib/i18n";

export default function ListingCard({ l, lang = "ru" }: { l: Listing; lang?: Lang }) {
  const photo = l.listing_photos?.[0]?.url;
  const idx = l.price_index ? INDEX_STYLE[l.price_index] : null;
  const meta =
    l.kind === "realty"
      ? [l.rooms ? `${l.rooms} ${t(lang, "u.rooms")}` : null, l.area ? `${l.area} м²` : null, l.floor].filter(Boolean).join(" · ")
      : "";
    ...
    >
      ...
      <div className="p-3">
        <div className="flex items-center justify-between gap-2">
          <span className="text-[15px] font-extrabold tracking-tight text-slate-900">{priceStr(l, lang)}</span>
          {idx && (
            <span
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-slate-400">
          {l.kind === "realty" && (
            ...
            >
              {l.owner_type === "owner" ? t(lang, "c.owner") : t(lang, "c.agency")}
            </span>
          )}
          {meta && <span>{meta}</span>}
        </div>
        <div className="mt-1.5 flex items-center gap-1 text-[11px] text-slate-400">
          <span>📍 {l.districts?.name_ru ?? t(lang, "city.tashkent")}</span>
        </div>
      </div>
    </Link>
  );
